...
void QTextDocumentPrivate::clear()
{
    ...
    for (auto curs : qAsConst(cursors)) {
        curs->setPosition(0);
...
void QTextDocumentPrivate::setLayout(QAbstractTextDocumentLayout *layout)
{
    ...
    if (lout == layout)
        return;
    const auto firstLayout = !lout;
    delete lout;
    lout = layout;

    if (!firstLayout)
        for (auto it = blocks.begin(); !it.atEnd(); ++it)
            it->free();
...
void QTextDocumentPrivate::insert_string(int pos, uint strPos, uint length, int format, QTextUndoCommand::Operation op)
{
    ...
    split(pos);
    auto x = fragments.insert_single(pos, length);
    auto X = fragments.fragment(x);
    X->format = format;
    X->stringPosition = strPos;
    auto w = fragments.previous(x);
    if (w)
        unite(w);
    ...
    auto frame = qobject_cast<QTextFrame *>(objectForFormat(format));
    if (frame) {
...
{
    split(pos);
    auto x = fragments.insert_single(pos, 1);
    auto X = fragments.fragment(x);
    X->format = format;
    X->stringPosition = strPos;
    ...
    auto B = blocks.fragment(b);
    B->format = blockFormat;

    Q_ASSERT(blocks.length() == fragments.length());

    auto group = qobject_cast<QTextBlockGroup *>(objectForFormat(blockFormat));
    if (group)
        group->blockInserted(QTextBlock(this, b));

    auto frame = qobject_cast<QTextFrame *>(objectForFormat(formats.format(format)));
    if (frame) {
        frame->d_func()->fragmentAdded(text.at(strPos), x);
        framesDirty = true;
    }
                                  ...
                                  int pos, int blockFormat, int charFormat, QTextUndoCommand::Operation op)
{
    ...
    auto strPos = text.length();
    text.append(blockSeparator);

    int ob = blocks.findNode(pos);
    auto atBlockEnd = true;
    auto atBlockStart = true;
    auto oldRevision = 0;
    if (ob) {
    ...
    const auto fragment = insert_block(pos, strPos, charFormat, blockFormat, op, QTextUndoCommand::BlockRemoved);
...
int QTextDocumentPrivate::remove_string(int pos, uint length, QTextUndoCommand::Operation op)
{
    ...
    auto frame = qobject_cast<QTextFrame *>(objectForFormat(fragments.fragment(x)->format));
    if (frame) {
...
int QTextDocumentPrivate::remove_block(int pos, int *blockFormat, int command, QTextUndoCommand::Operation op)
{
    ...
    *blockFormat = blocks.fragment(b)->format;
    ...
    auto group = qobject_cast<QTextBlockGroup *>(objectForFormat(blocks.fragment(b)->format));
    if (group)
...
void QTextDocumentPrivate::move(int pos, int to, int length, QTextUndoCommand::Operation op)
{
    ...
    auto dst = needsInsert ? fragments.findNode(to) : 0;
    auto dstKey = needsInsert ? fragments.position(dst) : 0;

    auto x = fragments.findNode(pos);
    auto end = fragments.findNode(pos+length);

    uint w = 0;
    while (x != end) {
        auto n = fragments.next(x);

        auto key = fragments.position(x);
        auto b = blocks.findNode(key+1);
        auto B = blocks.fragment(b);
        auto blockRevision = B->revision;
...
void QTextDocumentPrivate::setCharFormat(int pos, int length, const QTextCharFormat &newFormat, FormatChangeMode mode)
{
    ...
    if (pos == -1) {
        if (mode == MergeFormat) {
            auto format = formats.format(initialBlockCharFormatIndex);
    ...
    const auto endPos = pos + length;

    split(startPos);
    split(endPos);

    while (pos < endPos) {
        auto it = fragments.find(pos);
        Q_ASSERT(!it.atEnd());

        auto fragment = it.value();

        Q_ASSERT(formats.format(fragment->format).type() == QTextFormat::CharFormat);

        auto offset = pos - it.position();
        auto length = qMin(endPos - pos, int(fragment->size_array[0] - offset));
        auto oldFormat = fragment->format;

        if (mode == MergeFormat) {
            auto format = formats.format(fragment->format);
            format.merge(newFormat);
    ...
    auto blockIt = blocksFind(startPos);
    auto endIt = blocksFind(endPos);
    if (endIt.isValid())
        endIt = endIt.next();
                                          ...
                                          const QTextBlockFormat &newFormat, FormatChangeMode mode)
{
    ...
    if (mode == SetFormat)
        newFormatIdx = formats.indexForFormat(newFormat);
    auto group = qobject_cast<QTextBlockGroup *>(objectForFormat(newFormat));
    ...
    for (; it != end; it = it.next()) {
        auto oldFormat = block(it)->format;
        auto format = formats.blockFormat(oldFormat);
        auto oldGroup = qobject_cast<QTextBlockGroup *>(objectForFormat(format));
        if (mode == MergeFormat) {
            format.merge(newFormat);
...
//                k, (*it)->size_left[0], (*it)->size_array[0], pos);
        if (k != pos) {
            ...
            auto n = fragments.insert_single(pos, oldsize-(pos-k));
            X = fragments.fragment(x);
            auto N = fragments.fragment(n);
...
{
    auto n = fragments.next(f);
    if (!n)
        return false;

    auto ff = fragments.fragment(f);
    auto nf = fragments.fragment(n);
...
int QTextDocumentPrivate::undoRedo(bool undo)
{
    ...
    while (1) {
        ...
        case QTextUndoCommand::CharFormatChanged: {
            ...
            auto oldFormat = it.value()->format;
        ...
        case QTextUndoCommand::BlockFormatChanged: {
            ...
            auto it = blocksFind(c.pos);
            Q_ASSERT(it.isValid());

            auto oldFormat = block(it)->format;
            block(it)->format = c.format;
            auto oldGroup = qobject_cast<QTextBlockGroup *>(objectForFormat(formats.blockFormat(oldFormat)));
            auto group = qobject_cast<QTextBlockGroup *>(objectForFormat(formats.blockFormat(c.format)));
            c.format = oldFormat;
            if (group != oldGroup) {
                if (oldGroup)
                    oldGroup->blockRemoved(it);
        ...
        case QTextUndoCommand::GroupFormatChange: {
            ...
            auto object = objectForIndex(c.objectIndex);
            auto oldFormat = formats.objectFormatIndex(c.objectIndex);
...
void QTextDocumentPrivate::clearUndoRedoStacks(QTextDocument::Stacks stacksToClear,
                                               bool emitSignals)
{
    auto undoCommandsAvailable = undoState != 0;
    auto redoCommandsAvailable = undoState != undoStack.size();
    if (stacksToClear == QTextDocument::UndoStack && undoCommandsAvailable) {
        for (auto i = 0; i < undoState; ++i) {
            auto c = undoStack[undoState];
        ...
        for (auto i = undoState; i < undoStack.size(); ++i) {
            auto c = undoStack[i];
            if (c.command & QTextUndoCommand::Custom)
                delete c.custom;
        }
        ...
        if (emitSignals)
            emitRedoAvailable(false);
               ...
               && !undoStack.isEmpty()) {
        for (auto i = 0; i < undoStack.size(); ++i) {
            auto c = undoStack[i];
            if (c.command & QTextUndoCommand::Custom)
                delete c.custom;
        }
...
void QTextDocumentPrivate::endEditBlock()
{
    ...
    if (undoEnabled && undoState > 0) {
        const auto wasBlocking = !undoStack[undoState - 1].block_end;
...
//     qDebug("QTextDocumentPrivate::documentChange: from=%d,length=%d", from, length);
    ...
    auto start = qMin(from, docChangeFrom);
    auto end = qMax(from + length, docChangeFrom + docChangeLength);
    auto diff = qMax(0, end - start - docChangeLength);
    docChangeFrom = start;
...
//         qDebug("    -> %d %d %d", docChangeFrom, docChangeOldLength, docChangeLength);
    ...
    auto added = qMax(0, addedOrRemoved);
    auto removed = qMax(0, -addedOrRemoved);
    ...
    auto overlap_start = qMax(from, docChangeFrom);
    auto overlap_end = qMin(from + removed, docChangeFrom + docChangeLength);
    auto removedInside = qMax(0, overlap_end - overlap_start);
...
QString QTextDocumentPrivate::plainText() const
{
    ...
    auto text_unicode = text.unicode();
...
int QTextDocumentPrivate::nextCursorPosition(int position, QTextLayout::CursorMode mode) const
{
    ...
    auto it = blocksFind(position);
    auto start = it.position();
    auto end = start + it.length() - 1;
    if (position == end)
        return end + 1;
...
{
    beginEditBlock();
    auto objectIndex = obj->objectIndex();
    auto oldFormatIndex = formats.objectFormatIndex(objectIndex);
    formats.setObjectFormatIndex(objectIndex, format);

    auto b = qobject_cast<QTextBlockGroup *>(obj);
    if (b) {
        b->d_func()->markBlocksDirty();
    }
    auto f = qobject_cast<QTextFrame *>(obj);
    if (f)
...
static QTextFrame *findChildFrame(QTextFrame *f, int pos)
{
    ...
    const auto children = f->childFrames();
    auto first = 0;
    auto last = children.size() - 1;
    while (first <= last) {
        auto mid = (first + last) / 2;
        auto c = children.at(mid);
...
{
    for (auto i = 0; i < f->d_func()->childFrames.count(); ++i)
...
void QTextDocumentPrivate::scan_frames(int pos, int charsRemoved, int charsAdded)
{
    ...
    for (auto it = begin(); it != end(); ++it) {
        ...
        auto frame = qobject_cast<QTextFrame *>(objectForFormat(it->format));
        if (!frame)
            continue;

        Q_ASSERT(it.size() == 1);
        auto ch = text.at(it->stringPosition);
...
{
    auto start = f->firstPosition();
    auto end = f->lastPosition();
    auto parent = frameAt(start-1);
    Q_ASSERT(parent == frameAt(end+1));

    if (start != end) {
        // iterator over the parent and move all children contained in my frame to myself
        for (auto i = 0; i < parent->d_func()->childFrames.size(); ++i) {
            auto c = parent->d_func()->childFrames.at(i);
...
QTextFrame *QTextDocumentPrivate::insertFrame(int start, int end, const QTextFrameFormat &format)
{
    ...
    auto frame = qobject_cast<QTextFrame *>(createObject(format));
    Q_ASSERT(frame);

    // #### using the default block and char format below might be wrong
    auto idx = formats.indexForFormat(QTextBlockFormat());
    QTextCharFormat cfmt;
    cfmt.setObjectIndex(frame->objectIndex());
    auto charIdx = formats.indexForFormat(cfmt);
...
{
    auto parent = frame->d_func()->parentFrame;
    if (!parent)
        return;

    auto start = frame->firstPosition();
    auto end = frame->lastPosition();
    Q_ASSERT(end >= start);
...
QTextObject *QTextDocumentPrivate::objectForIndex(int objectIndex) const
{
    ...
    auto object = objects.value(objectIndex, 0);
    if (!object) {
        auto that = const_cast<QTextDocumentPrivate *>(this);
        auto fmt = formats.objectFormat(objectIndex);
...
{
    auto objectIndex = formats.format(formatIndex).objectIndex();
...
{
    auto obj = document()->createObject(f);
...
{
    const auto objIdx = object->d_func()->objectIndex;
...
void QTextDocumentPrivate::contentsChanged()
{
    ...
    auto m = undoEnabled ? (modifiedState != undoState) : true;
    if (modified != m) {
        modified = m;
...
void QTextDocumentPrivate::compressPieceTable()
{
    ...
    auto compressTable = unreachableCharacterCount * sizeof(QChar) > garbageCollectionThreshold
    ...
    auto newTextPtr = newText.data();
    auto newLen = 0;

    for (auto it = fragments.begin(); !it.atEnd(); ++it) {
...
bool QTextDocumentPrivate::ensureMaximumBlockCount()
{
    ...
    const auto blocksToRemove = blocks.numNodes() - maximumBlockCount;
    ...
    auto charFmt = cursor.blockCharFormat();
