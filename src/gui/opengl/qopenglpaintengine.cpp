...
GLuint QOpenGL2PaintEngineExPrivate::bindTexture(const QGradient &gradient)
{
    ...
    auto textureId = QOpenGL2GradientCache::cacheForContext(ctx)->getBuffer(gradient, 1.0);
...
//     qDebug("QOpenGL2PaintEngineExPrivate::updateBrushTexture()");
    ...
    if ( (style >= Qt::Dense1Pattern) && (style <= Qt::DiagCrossPattern) ) {
        ...
        auto textureImage = qt_imageForBrush(style, false);
        ...
        auto gradient = currentBrush.gradient();
...
//     qDebug("QOpenGL2PaintEngineExPrivate::updateBrushUniforms()");
    ...
    if (style == Qt::SolidPattern) {
        ...
        auto inv_matrix = gl_to_qt * (brushQTransform * matrix).inverted() * translate;
...
{
    auto c = (QOpenGL2PEVectorPathCache *) data;
#ifdef QT_OPENGL_CACHE_AS_VBOS
                                                ...
                                                QStaticTextItem *staticTextItem)
{
    ...
    if (recreateVertexArrays) {
        ...
        if (cache->hasPendingGlyphs()) {
            ...
            static const auto glypchCacheTextureUnit = QT_IMAGE_TEXTURE_UNIT;
    ...
    auto textureUnit = QT_MASK_TEXTURE_UNIT;
    if (glyphFormat == QFontEngine::Format_ARGB)
        textureUnit = QT_IMAGE_TEXTURE_UNIT;

    auto filterMode = (s->matrix.type() > QTransform::TxTranslate) ?
    ...
    auto updateMode = UpdateIfNeeded;
...
void QOpenGL2PaintEngineEx::setState(QPainterState *new_state)
{
    ...
    auto s = static_cast<QOpenGL2PaintEngineState *>(new_state);
