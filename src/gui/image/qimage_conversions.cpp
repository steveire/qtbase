...
void qGamma_correct_back_to_linear_cs(QImage *image)
{
    ...
    for (auto y=0; y<h; ++y) {
        auto pixels = (uint *) image->scanLine(y);
        for (auto x=0; x<w; ++x) {
            auto p = pixels[x];
            uint r = gamma[qRed(p)];
...
void convert_generic(QImageData *dest, const QImageData *src, Qt::ImageConversionFlags)
{
    ...
    auto srcLayout = &qPixelLayouts[src->format];
    auto destLayout = &qPixelLayouts[dest->format];
    ...
    for (auto y = 0; y < src->height; ++y) {
        auto x = 0;
        while (x < src->width) {
            auto l = qMin(src->width - x, buffer_size);
            auto ptr = fetch(buffer, srcData, x, l);
...
bool convert_generic_inplace(QImageData *data, QImage::Format dst_format, Qt::ImageConversionFlags)
{
    ...
    const auto buffer_size = 2048;
    uint buffer[buffer_size];
    auto srcLayout = &qPixelLayouts[data->format];
    auto destLayout = &qPixelLayouts[dst_format];
    auto srcData = data->data;
    ...
    for (auto y = 0; y < data->height; ++y) {
        auto x = 0;
        while (x < data->width) {
            auto l = qMin(data->width - x, buffer_size);
            auto ptr = fetch(buffer, srcData, x, l);
            ptr = convertToARGB32PM(buffer, ptr, l, srcLayout, 0);
            ptr = convertFromARGB32PM(buffer, ptr, l, destLayout, 0);
...
static void convert_passthrough(QImageData *dest, const QImageData *src, Qt::ImageConversionFlags)
{
    ...
    const auto src_pad = (src->bytes_per_line >> 2) - src->width;
    const auto dest_pad = (dest->bytes_per_line >> 2) - dest->width;
    auto src_data = (quint32 *) src->data;
    auto dest_data = (quint32 *) dest->data;

    for (auto i = 0; i < src->height; ++i) {
        auto end = src_data + src->width;
        while (src_data < end) {
            *dest_data = *src_data;
            ++src_data;
            ++dest_data;
...
static void convert_ARGB_to_ARGB_PM(QImageData *dest, const QImageData *src, Qt::ImageConversionFlags)
{
    ...
    const auto src_pad = (src->bytes_per_line >> 2) - src->width;
    const auto dest_pad = (dest->bytes_per_line >> 2) - dest->width;
    auto src_data = (QRgb *) src->data;
    auto dest_data = (QRgb *) dest->data;
...
Q_GUI_EXPORT void QT_FASTCALL qt_convert_rgb888_to_rgb32(quint32 *dest_data, const uchar *src_data, int len)
{
    ...
    for (; pixel + 3 < len; pixel += 4) {
        auto src_packed = (const quint32 *) src_data;
        const auto src1 = qFromBigEndian(src_packed[0]);
        const auto src2 = qFromBigEndian(src_packed[1]);
        const auto src3 = qFromBigEndian(src_packed[2]);
...
Q_GUI_EXPORT void QT_FASTCALL qt_convert_rgb888_to_rgbx8888(quint32 *dest_data, const uchar *src_data, int len)
{
    ...
    for (; pixel + 3 < len; pixel += 4) {
        auto src_packed = (const quint32 *) src_data;
        const auto src1 = src_packed[0];
        const auto src2 = src_packed[1];
        const auto src3 = src_packed[2];
...
{
    const auto a = rgb30 >> 30;
    switch (a) {
    case 0:
        return 0;
    case 1: {
        auto rgb = rgb30 & 0x3fffffff;
        rgb *= 3;
        return (a << 30) | rgb;
    }
    case 2: {
...
static void convert_A2RGB30_PM_to_RGB30(QImageData *dest, const QImageData *src, Qt::ImageConversionFlags)
{
    ...
    for (auto i = 0; i < src->height; ++i) {
        auto end = src_data + src->width;
        while (src_data < end) {
            const auto p = 0xc0000000 | qUnpremultiplyRgb30(*src_data);
...
static bool convert_A2RGB30_PM_to_RGB30_inplace(QImageData *data, Qt::ImageConversionFlags)
{
    ...
    const auto pad = (data->bytes_per_line >> 2) - data->width;
    auto rgb_data = (uint *) data->data;

    for (auto i = 0; i < data->height; ++i) {
        auto end = rgb_data + data->width;
        while (rgb_data < end) {
            const auto p = 0xc0000000 | qUnpremultiplyRgb30(*rgb_data);
...
static bool convert_indexed8_to_ARGB_PM_inplace(QImageData *data, Qt::ImageConversionFlags)
{
    ...
    const auto dst_bytes_per_line = ((data->width * depth + 31) >> 5) << 2;
    const auto nbytes = dst_bytes_per_line * data->height;
    const auto newData = (uchar *)realloc(data->data, nbytes);
    if (!newData)
        return false;
    ...
    auto src_data = newData + data->nbytes; // end of src
    auto dest_data = (quint32 *) (newData + nbytes); // end of dest > end of src
    const auto width = data->width;
    const auto src_pad = data->bytes_per_line - width;
    const auto dest_pad = (dst_bytes_per_line >> 2) - width;
    if (data->colortable.size() == 0) {
        data->colortable.resize(256);
        for (auto i = 0; i < 256; ++i)
            data->colortable[i] = qRgb(i, i, i);
    } else {
        for (auto i = 0; i < data->colortable.size(); ++i)
            data->colortable[i] = qPremultiply(data->colortable.at(i));

        // Fill the rest of the table in case src_data > colortable.size()
        const auto oldSize = data->colortable.size();
        const auto lastColor = data->colortable.at(oldSize - 1);
    ...
    for (auto i = 0; i < data->height; ++i) {
        src_data -= src_pad;
        dest_data -= dest_pad;
        for (auto pixI = 0; pixI < width; ++pixI) {
            --src_data;
            --dest_data;
...
static bool convert_indexed8_to_ARGB_inplace(QImageData *data, Qt::ImageConversionFlags)
{
    ...
    // start converting from the end because the end image is bigger than the source
    auto src_data = newData + data->nbytes;
    auto dest_data = (quint32 *) (newData + nbytes);
    const auto width = data->width;
    const auto src_pad = data->bytes_per_line - width;
    const auto dest_pad = (dst_bytes_per_line >> 2) - width;
    if (data->colortable.size() == 0) {
...
static bool convert_indexed8_to_RGB16_inplace(QImageData *data, Qt::ImageConversionFlags)
{
    ...
    // start converting from the end because the end image is bigger than the source
    auto src_data = newData + data->nbytes;
    auto dest_data = (quint16 *) (newData + nbytes);
    const auto width = data->width;
    const auto src_pad = data->bytes_per_line - width;
    const auto dest_pad = (dst_bytes_per_line >> 1) - width;
    ...
    if (data->colortable.isEmpty()) {
        ...
        const auto tableSize = data->colortable.size();
        ...
        const auto lastColor = colorTableRGB16[tableSize - 1];
        for (auto i = tableSize; i < 256; ++i)
...
static bool convert_RGB_to_RGB16_inplace(QImageData *data, Qt::ImageConversionFlags)
{
    ...
    const auto dst_bytes_per_line = ((data->width * depth + 31) >> 5) << 2;
    const auto src_bytes_per_line = data->bytes_per_line;
    auto src_data = (quint32 *) data->data;
    auto dst_data = (quint16 *) data->data;
    ...
    const auto newData = (uchar *)realloc(data->data, data->nbytes);
    if (newData) {
        data->data = newData;
        return true;
    } else {
...
static void convert_RGBA_to_RGB(QImageData *dest, const QImageData *src, Qt::ImageConversionFlags)
{
    ...
    const auto src_pad = (src->bytes_per_line >> 2) - src->width;
    const auto dest_pad = (dest->bytes_per_line >> 2) - dest->width;
    auto src_data = (const uint *)src->data;
...
{
    auto colorTable = ctbl;
    if (format == QImage::Format_RGB32) {
        // check if the color table has alpha
        for (auto i = 0; i < colorTable.size(); ++i)
                           ...
                           Qt::ImageConversionFlags flags, bool fromalpha)
{
    ...
    if (use_gray) {                                // make gray map
        if (fromalpha) {
            ...
            for (auto i = 0; i < src->colortable.size(); i++)
    ...
    auto dst_bpl = dst->bytes_per_line;
    auto src_data = src->data;
    auto src_bpl = src->bytes_per_line;

    switch (dithermode) {
    case Diffuse: {
        QScopedArrayPointer<int> lineBuffer(new int[w * 2]);
        auto line1 = lineBuffer.data();
        auto line2 = lineBuffer.data() + w;
        auto bmwidth = (w+7)/8;
        ...
        for (auto y=0; y<h; y++) {                        // for each scan line...
            auto tmp = line1; line1 = line2; line2 = tmp;
            auto not_last_line = y < h - 1;
            ...
            int err;
            auto p = dst->data + y*dst->bytes_per_line;
            memset(p, 0, bmwidth);
            b1 = line1;
            b2 = line2;
            auto bit = 7;
    ...
    if (dst->format == QImage::Format_MonoLSB) {
        ...
        auto sl = dst->data;
        auto bpl = (dst->width + 7) * dst->depth / 8;
        auto pad = dst->bytes_per_line - bpl;
...
static void convert_RGB_to_Indexed8(QImageData *dst, const QImageData *src, Qt::ImageConversionFlags flags)
{
    ...
    auto do_quant = (flags & Qt::DitherMode_Mask) == Qt::PreferDither
                       || src->format == QImage::Format_ARGB32;
    auto alpha_mask = src->format == QImage::Format_RGB32 ? 0xff000000 : 0;

    const auto tablesize = 997; // prime
    QRgbMap table[tablesize];
    auto pix=0;

    if (!dst->colortable.isEmpty()) {
        auto ctbl = dst->colortable;
        dst->colortable.resize(256);
        // Preload palette into table.
        // Almost same code as pixel insertion below
        for (auto i = 0; i < dst->colortable.size(); ++i) {
            // Find in table...
            auto p = ctbl.at(i) | alpha_mask;
    ...
    if ((flags & Qt::DitherMode_Mask) != Qt::PreferDither) {
        dst->colortable.resize(256);
        auto src_data = src->data;
        auto dest_data = dst->data;
        for (auto y = 0; y < src->height; y++) {        // check if <= 256 colors
            auto s = (const QRgb *)src_data;
            auto b = dest_data;
            for (auto x = 0; x < src->width; ++x) {
                auto p = s[x] | alpha_mask;
                int hash = p % tablesize;
                for (;;) {
                    if (table[hash].used) {
                        if (table[hash].rgb == (p)) {
    ...
    auto numColors = do_quant ? 256 : pix;
...
#define INDEXOF(r,g,b) (((r)*(MAX_G+1)+(g))*(MAX_B+1)+(b))
        ...
        for (auto rc=0; rc<=MAX_R; rc++)                // build 6x6x6 color cube
            for (auto gc=0; gc<=MAX_G; gc++)
                for (auto bc=0; bc<=MAX_B; bc++)
        ...
        if ((flags & Qt::Dither_Mask) == Qt::ThresholdDither) {
            for (auto y = 0; y < src->height; y++) {
                auto p = (const QRgb *)src_data;
                auto end = p + src->width;
                auto b = dest_data;
...
#undef DITHER
            ...
            for (auto y = 0; y < src->height; y++) {
                auto q = src_data;
                auto q2 = y < src->height - 1 ? q + src->bytes_per_line : src->data;
                auto b = dest_data;
                for (auto chan = 0; chan < 3; chan++) {
                    auto l1 = (y&1) ? line2[chan] : line1[chan];
                    auto l2 = (y&1) ? line1[chan] : line2[chan];
                    if (y == 0) {
                    ...
                    if (y&1) {
                        for (auto x = 0; x < src->width; x++) {
                            auto pix = qMax(qMin(5, (l1[x] * 5 + 128)/ 255), 0);
                            auto err = l1[x] - pix * 255 / 5;
                            pv[chan][x] = pix;
            ...
            for (auto y = 0; y < src->height; y++) {
                ...
                auto x = 0;
                while (p < end) {
                    auto d = qt_bayer_matrix[y & 15][x & 15] << 8;
...
static void convert_Indexed8_to_X32(QImageData *dest, const QImageData *src, Qt::ImageConversionFlags)
{
    ...
    auto colorTable = fix_color_table(src->colortable, dest->format);
    ...
    auto src_data = src->data;
    auto dest_data = dest->data;
    auto tableSize = colorTable.size() - 1;
    for (auto y = 0; y < src->height; y++) {
        auto p = (uint *)dest_data;
        auto b = src_data;
        auto end = p + w;
...
static void convert_Indexed8_to_Alpha8(QImageData *dest, const QImageData *src, Qt::ImageConversionFlags)
{
    ...
    auto simpleCase = (colors.size() == 256);
    ...
    if (simpleCase)
        ...
        auto size = src->bytes_per_line * src->height;
