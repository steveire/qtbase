...
{
    auto reciprocal = 1 / qSqrt(x * x + y * y);
    x *= reciprocal;
    y *= reciprocal;
}
...
bool QIntersectionFinder::linesIntersect(const QLineF &a, const QLineF &b) const
{
    ...
    const auto p1_equals_q1 = comparePoints(p1, q1);
    const auto p2_equals_q2 = comparePoints(p2, q2);

    if (p1_equals_q1 && p2_equals_q2)
        return true;

    const auto p1_equals_q2 = comparePoints(p1, q2);
    const auto p2_equals_q1 = comparePoints(p2, q1);

    if (p1_equals_q2 && p2_equals_q1)
        return true;

    const auto pDelta = p2 - p1;
    const auto qDelta = q2 - q1;

    const auto par = pDelta.x() * qDelta.y() - pDelta.y() * qDelta.x();
    ...
    if (qFuzzyIsNull(par)) {
        ...
        if (qFuzzyIsNull(dot(normal, q1 - p1))) {
            const auto dp = dot(pDelta, pDelta);

            const auto tq1 = dot(pDelta, q1 - p1);
            const auto tq2 = dot(pDelta, q2 - p1);

            if ((tq1 > 0 && tq1 < dp) || (tq2 > 0 && tq2 < dp))
                return true;

            const auto dq = dot(qDelta, qDelta);

            const auto tp1 = dot(qDelta, p1 - q1);
            const auto tp2 = dot(qDelta, p2 - q1);
    ...
    const auto invPar = 1 / par;

    const auto tp = (qDelta.y() * (q1.x() - p1.x()) -
                      qDelta.x() * (q1.y() - p1.y())) * invPar;

    if (tp < 0 || tp > 1)
        return false;

    const auto tq = (pDelta.y() * (q1.x() - p1.x()) -
...
bool QIntersectionFinder::hasIntersections(const QPathSegments &a, const QPathSegments &b) const
{
    ...
    auto minY = rb0.top();
    auto maxX = rb0.right();
    auto maxY = rb0.bottom();

    for (auto i = 1; i < b.segments(); ++i) {
      ...
      m_intersections(0)
{
    ...
    auto root = buildTree(0, m_index.size(), 0, m_bounds);
    m_tree[0] = root;
}
...
TreeNode SegmentTree::buildTree(int first, int last, int depth, const RectF &bounds)
{
    ...
    auto splitAxis = (depth & 1);

    TreeNode node;
    node.leaf = false;

    auto split = 0.5f * ((&bounds.x1)[splitAxis] + (&bounds.x2)[splitAxis]);
    ...
    const auto treeSize = m_tree.size();
    ...
    while (l <= r) {
        const auto index = m_index.at(l);
        const QRectF &segmentBounds = m_segments.elementBounds(index);

        auto lowCoordinate = coordinate(segmentBounds.topLeft(), splitAxis);

        if (coordinate(segmentBounds.center(), splitAxis) < split) {
            auto highCoordinate = coordinate(segmentBounds.bottomRight(), splitAxis);
    ...
    auto left = buildTree(first, l, depth + 1, lbounds);
    m_tree[node.index.children.left] = left;

    auto right = buildTree(l, last, depth + 1, rbounds);
...
void SegmentTree::intersectLines(const QLineF &a, const QLineF &b, QDataBuffer<QIntersection> &intersections)
{
    ...
    if (qFuzzyIsNull(par)) {
        ...
        // coinciding?
        if (qFuzzyIsNull(dot(normal, q1 - p1))) {
            const auto invDp = 1 / dot(pDelta, pDelta);

            const auto tq1 = dot(pDelta, q1 - p1) * invDp;
            const auto tq2 = dot(pDelta, q2 - p1) * invDp;
            ...
            const auto invDq = 1 / dot(qDelta, qDelta);

            const auto tp1 = dot(qDelta, p1 - q1) * invDq;
            const auto tp2 = dot(qDelta, p2 - q1) * invDq;
    ...
    const auto p_zero = qFuzzyIsNull(tp);
    const auto p_one = qFuzzyIsNull(tp - 1);

    const auto q_zero = qFuzzyIsNull(tq);
    const auto q_one = qFuzzyIsNull(tq - 1);
...
void SegmentTree::produceIntersectionsLeaf(const TreeNode &node, int segment)
{
    ...
    const auto lineA = m_segments.lineAt(segment);

    for (auto i = node.index.interval.first; i < node.index.interval.last; ++i) {
        const auto other = m_index.at(i);
        if (other >= segment)
            continue;
        ...
        const auto lineB = m_segments.lineAt(other);

        intersectLines(lineA, lineB, m_intersections);

        for (auto k = 0; k < m_intersections.size(); ++k) {
...
void SegmentTree::produceIntersections(const TreeNode &node, int segment, const RectF &segmentBounds, const RectF &nodeBounds, int axis)
{
    ...
    auto lbounds = nodeBounds;
    (&lbounds.x2)[axis] = node.splitLeft;

    auto rbounds = nodeBounds;
...
void QIntersectionFinder::produceIntersections(QPathSegments &segments)
{
    ...
    for (auto i = 0; i < segments.segments(); ++i)
...
void qTraverseKdPointTree(QKdPointTree::Node &node, T &t, int depth = 0)
{
    ...
    const auto traverseRight = (status == QKdPointTree::TraverseBoth || status == QKdPointTree::TraverseRight);
    const auto traverseLeft = (status == QKdPointTree::TraverseBoth || status == QKdPointTree::TraverseLeft);
...
int QKdPointTree::build(int begin, int end, int depth)
{
    ...
    const auto pivot = component(m_segments->pointAt(m_nodes.at(begin).point), depth & 1);

    auto first = begin + 1;
    auto last = end - 1;

    while (first <= last) {
        const auto value = component(m_segments->pointAt(m_nodes.at(first).point), depth & 1);
...
public:
    ...
    inline QKdPointTree::Traversal operator()(QKdPointTree::Node &node, int depth)
    {
        ...
        const auto pivot = pivotComponents[depth & 1];
        const auto value = pointComponents[depth & 1];

        if (fuzzyIsNull(pivot - value)) {
            const auto pivot2 = pivotComponents[(depth + 1) & 1];
            const auto value2 = pointComponents[(depth + 1) & 1];
...
void QPathSegments::mergePoints()
{
    ...
    if (tree.rootNode()) {
        ...
        for (auto i = 0; i < m_intersections.size(); ++i)
...
void QWingedEdge::intersectAndAdd()
{
    ...
    for (auto i = 0; i < m_segments.points(); ++i)
        addVertex(m_segments.pointAt(i));

    QDataBuffer<QPathSegments::Intersection> intersections(m_segments.segments());
    for (auto i = 0; i < m_segments.segments(); ++i) {
        intersections.reset();

        auto pathId = m_segments.pathId(i);

        auto isect = m_segments.intersectionAt(i);
        while (isect) {
            intersections << *isect;
        ...
        auto first = m_segments.segmentAt(i).va;
        auto second = m_segments.segmentAt(i).vb;

        auto last = first;
        for (auto j = 0; j < intersections.size(); ++j) {
            const QPathSegments::Intersection &isect = intersections.at(j);

            auto ep = edge(addEdge(last, isect.vertex));

            if (ep) {
                const auto dir = m_segments.pointAt(last).y() < m_segments.pointAt(isect.vertex).y() ? 1 : -1;
                if (pathId == 0)
                    ep->windingA += dir;
                else
                    ep->windingB += dir;
        ...
        auto ep = edge(addEdge(last, second));

        if (ep) {
            const auto dir = m_segments.pointAt(last).y() < m_segments.pointAt(second).y() ? 1 : -1;
            if (pathId == 0)
                ep->windingA += dir;
            else
                ep->windingB += dir;
...
{
    const auto equal_1_2 = comparePoints(bezier.pt1(), bezier.pt2());
    const auto equal_2_3 = comparePoints(bezier.pt2(), bezier.pt3());
    const auto equal_3_4 = comparePoints(bezier.pt3(), bezier.pt4());
...
{
    auto firstSegment = m_segments.size();

    auto hasMoveTo = false;
    auto lastMoveTo = 0;
    auto last = 0;
    for (auto i = 0; i < path.elementCount(); ++i) {
        auto current = m_points.size();
        ...
        case QPainterPath::CurveToElement:
            {
                auto bezier = QBezier::fromPoints(m_points.at(last), path.elementAt(i), path.elementAt(i+1), path.elementAt(i+2));
                if (isLine(bezier)) {
                    m_segments << Segment(m_pathId, last, current);
                } else {
    ...
    for (auto i = firstSegment; i < m_segments.size(); ++i) {
...
qreal QWingedEdge::delta(int vertex, int a, int b) const
{
    ...
    auto a_angle = ap->angle;
    auto b_angle = bp->angle;
...
QWingedEdge::TraversalStatus QWingedEdge::findInsertStatus(int vi, int ei) const
{
    ...
    do {
        ...
        auto d2 = delta(vi, ei, status.edge);
...
void QWingedEdge::removeEdge(int ei)
{
    ...
    auto forwardRight = next(status);
    forwardRight.flipDirection();

    status.traversal = QPathEdge::LeftTraversal;
    auto forwardLeft = next(status);
    forwardLeft.flipDirection();

    status.direction = QPathEdge::Backward;
    auto backwardLeft = next(status);
    backwardLeft.flipDirection();

    status.traversal = QPathEdge::RightTraversal;
    auto backwardRight = next(status);
    ...
    auto a = vertex(ep->first);
    auto b = vertex(ep->second);
...
{
    auto ap = list.vertex(a);
    Q_ASSERT(ap);

    auto bp = list.vertex(b);
    Q_ASSERT(bp);
...
int QWingedEdge::addEdge(int fi, int si)
{
    ...
    auto common = commonEdge(*this, fi, si);
    if (common >= 0)
        return common;

    m_edges << QPathEdge(fi, si);

    auto ei = m_edges.size() - 1;
    ...
    const auto tangent = QPointF(*sp) - QPointF(*fp);
    ...
    for (auto i = 0; i < 2; ++i) {
        ...
        if (vp->edge < 0) {
            ...
            auto vi = ep->vertex(dirs[i]);
            Q_ASSERT(vertex(vi) == vertices[i]);

            auto os = findInsertStatus(vi, ei);
            auto op = edge(os.edge);

            Q_ASSERT(vertex(op->vertex(os.direction)) == vertices[i]);

            auto ns = next(os);
            ns.flipDirection();
            auto np = edge(ns.edge);
...
{
    const auto elementCount = path.elementCount();
    if (elementCount >= 2) {
        ...
        if (middle.type == QPainterPath::LineToElement) {
            ...
            const auto d1 = point - first;
            const auto d2 = middle - first;
...
bool QPathClipper::intersect()
{
    ...
    auto r1 = subjectPath.controlPointRect();
    auto r2 = clipPath.controlPointRect();
    ...
    auto subjectIsRect = pathToRect(subjectPath);
    auto clipIsRect = pathToRect(clipPath);
    ...
    for (auto i = 0; i < clipPath.elementCount(); ++i) {
    ...
    for (auto i = 0; i < subjectPath.elementCount(); ++i) {
...
bool QPathClipper::pathToRect(const QPainterPath &path, QRectF *rect)
{
    ...
    const auto mightBeRect = path.elementAt(0).isMoveTo()
    ...
    const auto x1 = path.elementAt(0).x;
    const auto y1 = path.elementAt(0).y;

    const auto x2 = path.elementAt(1).x;
    const auto y2 = path.elementAt(2).y;
...
QPainterPath QPathClipper::clip(Operation operation)
{
    ...
    if (op != Simplify) {
        ...
        auto subjectIsRect = pathToRect(subjectPath, 0);
        auto clipIsRect = pathToRect(clipPath, 0);

        const auto clipBounds = clipPath.boundingRect();
        const auto subjectBounds = subjectPath.boundingRect();
            ...
            if (subjectIsRect) {
                ...
                case BoolSub:
                    if (clipPath.fillRule() == Qt::OddEvenFill) {
                        auto result = clipPath;
                        result.addRect(subjectBounds);
                        return result;
                    } else {
                        auto result = clipPath.simplified();
                        result.addRect(subjectBounds);
                        return result;
                    }
                case BoolAnd:
...
bool QPathClipper::doClip(QWingedEdge &list, ClipperMode mode)
{
    ...
    for (auto i = 0; i < list.vertexCount(); ++i)
    ...
    do {
        ...
        auto index = 0;
        qreal maxHeight = 0;
        for (auto i = 0; i < list.edgeCount(); ++i) {
            auto edge = list.edge(i);

            // have both sides of this edge already been handled?
            if ((edge->flag & 0x3) == 0x3)
                continue;

            auto a = list.vertex(edge->first);
            auto b = list.vertex(edge->second);
        ...
        if (found) {
            ...
            auto bestY = 0.5 * (y_coords[first] + y_coords[first+1]);
            auto biggestGap = y_coords[first+1] - y_coords[first];

            for (auto i = first + 1; i < last; ++i) {
                auto gap = y_coords[i+1] - y_coords[i];
...
static void traverse(QWingedEdge &list, int edge, QPathEdge::Traversal traversal)
{
    ...
    do {
        auto flag = status.traversal == QPathEdge::LeftTraversal ? 1 : 2;
...
bool QWingedEdge::isInside(qreal x, qreal y) const
{
    ...
    for (auto i = 0; i < edgeCount(); ++i) {
        ...
        if ((a.y() < y && b.y() > y) || (a.y() > y && b.y() < y)) {
            auto intersectionX = a.x() + (b.x() - a.x()) * (y - a.y()) / (b.y() - a.y());
...
static QVector<QCrossingEdge> findCrossings(const QWingedEdge &list, qreal y)
{
    ...
    for (auto i = 0; i < list.edgeCount(); ++i) {
        ...
        if ((a.y() < y && b.y() > y) || (a.y() > y && b.y() < y)) {
            const auto intersection = a.x() + (b.x() - a.x()) * (y - a.y()) / (b.y() - a.y());
...
{
    auto crossings = findCrossings(list, y);
...
#endif
    for (auto i = 0; i < crossings.size() - 1; ++i) {
        auto ei = crossings.at(i).edge;
        auto edge = list.edge(ei);
        ...
        const auto inA = (windingA & aMask) != 0;
        const auto inB = (windingB & bMask) != 0;
        const auto inD = (windingD & 0x1) != 0;

        const auto inside = bool_op(inA, inB, op);
        ...
        if (add) {
            ...
            auto y0 = list.vertex(edge->first)->y;
            auto y1 = list.vertex(edge->second)->y;
...
{
    auto bezier = QBezier::fromPoints(a, b, c, d);
    ...
    auto outCount = int(outA) + int(outB) + int(outC) + int(outD);
    ...
    auto flip = isVertical(edge) ? QTransform(0, 1, 1, 0, 0, 0) : QTransform();
    auto unflipped = bezier;
    auto flipped = bezier.mapBy(flip);

    qreal t0 = 0, t1 = 1;
    auto stationary = flipped.stationaryYPoints(t0, t1);
    ...
    for (auto i = 0; i < segmentCount; ++i) {
        ...
        if (outA != outB) {
            auto intersection = flipped.tForY(segments[i], segments[i+1], t);
...
QPainterPath intersectPath(const QPainterPath &path, const QRectF &rect)
{
    ...
    for (auto i = 0; i < subpaths.size(); ++i) {
        auto subPath = subpaths.at(i);
        auto bounds = subPath.boundingRect();
